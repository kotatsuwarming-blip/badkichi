...
--
-- 信頼性レベル:
-- - 🔵 青信号: 要件定義・確定スキーマ(initial_schema.sql)・ヒアリングを参考にした確実な定義
-- - 🟡 黄信号: 妥当な推測による定義
-- - 🔴 赤信号: 出典のない推測
--
-- 重要方針:
-- - 既存テーブルのスキーマは変更しない（列追加・新規テーブルなし, REQ-402）。本ファイルは
--   実装時に supabase/migrations/<ts>_stats_dashboard_read_functions.sql として配置し、
--   関数 + GRANT のみを additive 追加する。適用は CI 経由 db:push（ローカル不可, REQ-408 / [[feedback_db_password_ci_only]]）。
-- - 全関数は SECURITY INVOKER + STABLE。RLS（rallies/sets/matches の FK 経由 is_member_of）を
--   継承するため、他 Group のデータは集計・返却されない（REQ-403 / NFR-101）。
-- - 全関数に SET search_path = public（search_path 攻撃防御。既存 SECURITY 関数と方針統一）。
-- - スコープ引数は p_match_id / p_group_id の「いずれか一方のみ」必須。両方 NULL / 両方指定は invalid_scope。
-- - 確定ラリーの定義（得点率の集計対象, REQ-101）:
...
BEGIN
  ...
  RETURN QUERY
  WITH confirmed AS (
    -- 🔵 確定ラリーのみ（REQ-101）。RLS は SECURITY INVOKER で継承
...
--    ペアは試合のチーム構成から無向 2 選手集合として導出（同じ 2 人が複数試合で組めば累計）。
--    ペアの所属チームが serving_team のラリー = serve、相手が serving = receive。
-- ========================================
...
RETURNS TABLE (
  player1_id uuid,  -- 🔵 正規化: LEAST(uuid)
  player2_id uuid,  -- 🔵 正規化: GREATEST(uuid)
  serve_total bigint,
  serve_won bigint,
  receive_total bigint,
  receive_won bigint
...
-- ========================================
-- 3. stats_rally_length: ラリー長別 本数分布 + サーブ側勝数 🔵 REQ-005 / ヒアリング2026-06-08
...
-- ========================================
-- 4. stats_rallies: ラリー行（テーブル / クロスフィルタ / 再生用） 🔵 REQ-006/007/010/104
...
CREATE OR REPLACE FUNCTION stats_rallies(
  ...
  p_role text DEFAULT NULL,            -- 🔵 'serve' | 'receive'（選手・ペア絞り込みの役割連動, ヒアリング2026-06-09）
  p_shot_ranges jsonb DEFAULT NULL,    -- 🔵 [{"min":1,"max":3},{"min":13,"max":null}] 形式の OR 範囲（複数ビン, ヒアリング2026-06-09）
...
BEGIN
  ...
  WHERE r.deleted_at IS NULL
    AND (p_match_id IS NULL OR m.id = p_match_id)
    AND (p_group_id IS NULL OR m.group_id = p_group_id)
    -- 🔵 1 選手絞り込み（player_id・role 連動）: role に応じてサーバー/レシーバーを見る（未指定なら両方）
    ...
    -- 🔵 ラリー長: 選択ビンの和集合（OR）。shot_count を各範囲と突き合わせ（ヒアリング2026-06-09）
    AND (
    ...
    -- 🔵 ペア絞り込み（役割連動, ヒアリング2026-06-09）:
    --    role='serve'   → ペアが serving 側チームを構成するラリー
    --    role='receive' → ペアが受け側チームを構成するラリー
    --    role=NULL       → ペアが出場（どちらかのチーム）の全ラリー
    AND (
  ...
  LIMIT GREATEST(0, LEAST(COALESCE(p_limit, 200), 1000))   -- 🟡 上限 1000 で暴走防止
  OFFSET GREATEST(0, COALESCE(p_offset, 0));
END;
$$;

-- ========================================
-- GRANT（authenticated ロールにのみ実行権限） 🔵 Supabase RPC 公開規約
-- ========================================
...
-- 信頼性レベルサマリー
-- ========================================
-- - 🔵 青信号: 約 95%（確定スキーマ・要件・ヒアリングに準拠。ペア役割連動・ラリー長ビン OR も確定）
-- - 🟡 黄信号: 約 5%（limit 上限 1000 の安全弁のみ）
-- - 🔴 赤信号: 0 件
--
-- 品質評価: 高品質
