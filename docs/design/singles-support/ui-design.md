...
※ ダブルスロングサービスライン（バックから 0.76m）は実コートに常在するため 3 図とも**常に**描く（2026-09-16 ヒアリングで注釈タップコート・記録コート図にも追加。統計ゾーン図は既存）。
- `CourtDiagramInput`: y = COURT.top + 7.6 / COURT.top + COURT.height − 7.6 の横線（シングルスサイドラインと同トーン）
- `CourtDiagram`（記録）: 上下 5.7%（76/1340）内側に横の白線オーバーレイ（形式問わず常時）
