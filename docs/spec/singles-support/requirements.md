...
### 含む（2026-09-14 ヒアリングで追加）
- シングルスサイドライン（内側の縦ライン）のコート図描画（注釈タップコート・統計ゾーン図・記録コート図）
- 失点マップ（StatsWeaknessMaps）のシングルス時の注記切替（シングルスは選手単位の集計として表示）
...
- REQ-111: 対戦形式がシングルスの場合、クイックパスの落下点整合チェック（EDGE-002 ソフト警告）は in/out の境界を**シングルスサイドライン**としなければならない（ダブルスサイドラインとシングルスサイドラインの間のタップは out 扱い = 「アウトなのにコート内」警告を出さない） 🔵 *ヒアリング 2026-09-16 + BWF 競技規則*
- REQ-112: 決着がサーブ（ラリー 1 打）の場合、同整合チェックは**ショートサービスライン手前**（ダブルスではロングサービスライン奥も）を out 扱いとしなければならない（サーブの届かないアウトで警告を出さない） 🔵 *ヒアリング 2026-09-16 + BWF 競技規則 §9（サーブはショートサービスラインを越えて着地）*
...
- REQ-301: システムはシングルスサイドライン（ダブルスサイドラインから 0.46m 内側の縦ライン）をコート図に描かなければならない 🔵 *ヒアリング 2026-09-14（位置タップの精度向上のため実装）+ BWF 競技規則コート寸法*
  - REQ-301a: 注釈の位置タップコート（CourtDiagramInput）と統計ゾーン図（StatsCourtZones）では、実コートと同様に**常に**描く（実コートには常に線が引かれており、形式に関わらずタップの目印になる）
  - REQ-301b: 記録画面のコート図（CourtDiagram）では、シングルス時にサイドラインを描く（外周 = ダブルスサイドラインとの区別）
  - REQ-301c: ダブルスロングサービスライン（バックバウンダリーから 0.76m 内側の横線）を、統計ゾーン図に加えて注釈タップコート・記録コート図にも**常に**描く（実コートに常在する線。2026-09-16 ヒアリング追加）
- REQ-302: システムは失点マップ（被決定点）をシングルスでは選手単位の集計として注記しなければならない（ダブルス向けの「ペア両選手で共有」注記を出さない） 🔵 *ヒアリング 2026-09-14。シングルスはチーム = 個人のため集計自体は既に個人単位*
