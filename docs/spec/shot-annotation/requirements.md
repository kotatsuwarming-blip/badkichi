...
- REQ-002: システムは、additive migration 1本で以下の列を追加しなければならない 🔵 *ADR-017 §5*
  - `shots`: `hit_player_id` / `shot_type` / `hand` / `hit_x` / `hit_y` / `hit_height`（high / low、対象球種のみ任意入力、REQ-110）/ `annotation_source` / `ai_model_version` / `ai_confidence`
...
- REQ-110: システムは、打点の高さ（`hit_height`: `high` / `low`、null = 未入力）を**任意入力**として提供しなければならない。対象はオーバーヘッド系（クリア / スマッシュ / カット / リバースカット / ドロップ）・プッシュ・サーブを**除く**球種（ヘアピン / ロブ / ドライブ / ハーフ / レシーブ3種）のみとし、対象外の球種では入力 UI を提示しない（高さが球種から自明のため）。入力手段は種別パスの矢印修飾（種別キー → `↑` / `↓`）と打点パスのタップ + 縦フリック。高低の判断基準は球種グループごとに定める: **ネット前系**（ヘアピン・ショートレシーブ）= 白帯より上で触れたか / **フラット系**（ドライブ・ドライブレシーブ・ハーフ）= 肩〜腰より上か / **上げ系**（ロブ・ロングレシーブ）= 腰より上で早く触れたか。連続値の高さ（z）は入力させず Stage 3（姿勢推定）で自動導出する 🔵 *ヒアリング2026-07-24（必須にしない・オーバーヘッドとプッシュ以外で価値: ネット前の高さ / 後衛の高低 / レシーブ高低別の苦手分析）*
