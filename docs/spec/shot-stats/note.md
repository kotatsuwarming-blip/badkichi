...
- 全体フィルタ（グローバル/打者/セット）の整合性整理は保留。
- **ポジション別得点率（StatsRateChart）だけ打者フィルタが利かない**（2026-08-08 指摘）:
  旧概要由来でグローバル対象フィルタ連動のため全員分が表示される。全員比較自体は
  面白いという評価なので、**別画面/機能（選手比較ビュー等）として切り出しを検討**。
