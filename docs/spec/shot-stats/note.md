...
**コートチェンジ（チェンジエンズ）手動対応（2026-08-22、match-recording 側）**:
- ファイナルゲーム 11 点（15 点マッチは 8 点）等のコートチェンジは従来未対応で、
  camera_near_team がセット開始時の値のまま carry され、チェンジ後の座標系集計
  （ヒートマップ/弱点分析/レシーブコース）が左右前後逆に算入される問題があった。
- 記録画面に**「コートチェンジ」ボタン**（手動トグル）を追加。以降のラリーの
  camera_near_team を反転。練習で交代しない/1ゲームだけで交代する等の変則運用にも対応。
- 再開（リロード）時は最後のラリーの camera_near_team を引き継ぐ（at_start に戻らない）。
- 自動化（ファイナルフラグ + 11 点で自動反転）は運用を見て将来判断。
