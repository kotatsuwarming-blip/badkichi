...
- キーボード配置 (案): 数字段 = 通常ショット (1 クリア / 2 スマッシュ / 3 カット /
  ...
  QWE = レシーブ3種。
- **フォア/バックはパス単位の宣言トグル + 修飾キー** (追補 2026-07-24)。トグル ON の
  パスでは無印キー = forehand を**明示保存**、Shift+種別キー (スマホ: 長押し) = backhand。
  トグル OFF では書き込まない (null = 未判定)。フォアも明示値として保存するため、
  null が「フォアか未入力か不明」になる曖昧さがない。必須タップは増えない。
  将来 Stage 3 の姿勢推定 + 選手の利き手から導出可能になったら手動分は検証データになる。
- **打点の高さ (z) は手動入力しない — MVP 対象外** (追補 2026-07-24)。高さの変動は
  全球種にあり (低い打点のドロップ・ジャンプスマッシュ等) 分析価値も高いが、だからこそ
  球種限定の例外入力では拾えず、全ショット対象では種別・打点に続く第3の入力軸として
  手動コストが重すぎる。**連続値の z を Stage 3 (姿勢推定) で全ショット自動導出**する。
...
値の定義 (シャトルの最終結果。互いに排他・動画から客観判定可能。**6値、追補 2026-08-02**):

| 値 | 定義 | 勝敗 (打者 = 最後にラケットで触れた選手) |
|---|---|---|
| `floor` | **ネットを越えて床に落ちた** (イン/アウトは入力しない — 下記の導出) | point_winner から導出: 打者 = 勝者 → **in** / 打者 = 敗者 → **out** |
...
| `unknown` | 判定不能。**正規の選択肢** (強制ラベルはノイズ源) | — |

**in/out の統合 (追補 2026-08-02、ドッグフーディングの発見)**: 決着ループを見ても
イン/アウトは動画から判定できない (PRD 注記どおり)。一方で
**in/out = 最終接触者 + point_winner (ライブ記録の実測値) から機械的に導出できる**
(越えて床に落ちた球は、最後に打った側が得点していればイン、失点していればアウト)。
よって人間には判定可能な `floor` だけを入力させ、in/out は集計時導出とする。
当初の in/out 2値は移行 migration で floor へ変換済み (導出で再現可能なため情報損失なし)。

- out の**サイドアウト / バックアウト / 両方 (コーナー)** は落下点座標から導出する (§5)。
  落下点をスキップした場合のみサブ選択 (`out_direction`: side / back / both) でフォールバック
  (サブ選択は導出が out のときだけ提示)。
- net と not_over の境界は「ネットに触れたか」で客観判定。どちらも打者の失点なので、
  仮に迷っても勝敗の導出は壊れない。
- サーブが**ネット / 床 (越えて外)** になった場合は物理結果どおり net / floor を使う
...
- **整合チェック**: (最終接触者のチーム, end_reason) から導出される勝者と、記録済みの
  point_winner が矛盾したら入力ミスとして警告できる (floor / unknown は導出不能のため
  対象外 — floor は逆に point_winner を使って in/out を導出する側)。
- **決定打 = 勝者チームの最後のショット**として導出する (floor(in) / body なら最終ショット、
  floor(out) / net / not_over ならその1つ前 = 相手のミスを強いたショット)。専用の入力は不要。
