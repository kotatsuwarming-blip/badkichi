...
- **打点が映像に見えないショットのデータ化**（2026-08-05 提案。テスト動画は手前コート左側が
  見切れており打点・タイミングが判定できないショットがある。「見えない」を skip (null) と
  区別して記録したい → 案: shots に hit_visibility ('visible'/'occluded') 列を追加し、
  パネルに「見えない」ボタン。統計側で excludable に。要件確認後に実装）
