...
- 種別語彙: unknown (U) 追加（2026-08-03）、**lob → lob_high (0) / lob_low (L)、
  clear → clear_high (1) / clear_driven (D) に分割**（2026-08-05。旧 'lob'/'clear' は
  レガシー値として DB/表示は許容、パレットから撤去。既存データはユーザーが手で振り分け予定）
