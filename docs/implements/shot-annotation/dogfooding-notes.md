...
- 種別語彙 18種: unknown (U) 追加（2026-08-03）、**lob → lob_high (0) / lob_low (L)、
  clear → clear_high (1) / clear_driven (D) に分割**（2026-08-05）。通し計測用の振り分けは
  完了し、レガシー値 'lob'/'clear' は**コード・DB CHECK とも撤去済み**（migration
  20260805140000。残存23行はユーザー確認の上 NULL 化。旧ドッグフーディング3試合は削除済み、
  オリジナル 0433d0f5 はライブ記録の原本として残置）
- **種別候補の対応表カバー率: 90.9%**（通し計測用 186打中169打、2026-08-05 計測）。
  主な取りこぼし: mid→rear の push (4) / rear→rear の smash (4) / mid→rear の clear_high (2)
...
- **映像の見切れ情報（2026-08-05 方針確定・実装は保留）**: テスト動画は手前コート左側が
  見切れており、該当箇所の打点・打刻は不正確な可能性がある。持ち方は**試合データ単位**の
  メタ情報（「この試合の映像は一部見切れがあり、タイムスタンプが正確でない可能性がある」
  程度の粒度）でよい。ショット単位のフラグや統計表示の変更は不要。**今は実装しない**
