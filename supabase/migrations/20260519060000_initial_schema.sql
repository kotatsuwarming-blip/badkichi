...
-- ========================================
-- RLS ヘルパー関数 (TASK-0006)
-- ========================================

-- 🔵 RLS ヘルパー: ログイン中ユーザーが指定 group_id に所属しているか (ヒアリング 2026-04-20 Q2)
-- 🔵 B2 確定方針 (2026-05-13): SECURITY DEFINER 関数では search_path 攻撃防御として
--    `SET search_path = public` を必ず付与する。
...
-- ========================================
-- Row Level Security (RLS) (TASK-0006)
-- ========================================
...
-- RPC 関数 (TASK-0007)
-- ========================================
-- 🔵 B1 確定方針 (2026-05-13): TASK-0005 で作成した本マイグレーションファイルに追記する。
--    新規 _rpc.sql は作成しない。
-- 🔵 B2 確定方針 (2026-05-13): 全 SECURITY DEFINER 関数に `SET search_path = public` を
--    必ず付与する (search_path 攻撃防御)。is_member_of() と方針を揃える。
