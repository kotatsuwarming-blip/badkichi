...
describe('useSetRallies', () => {
  beforeEach(() => {
    ...
    orderMock.mockResolvedValue({
      data: [
        { rally_number: 1, serving_team: 'A', server_player_id: 'p1', point_winner: 'A', is_let: false, is_point_confirmed: true, shots: [{ count: 3 }] },
        { rally_number: 2, serving_team: 'A', server_player_id: 'p2', point_winner: null, is_let: false, is_point_confirmed: false, shots: [] }
      ],
      error: null
    })
  })
  ...
  it('TC1: RallyHistoryItem へ射影し shotCount を埋め込み count から取る', async () => {
    ...
    expect(data.value?.[0]).toEqual({
      ...
      isLet: false, isPointConfirmed: true, shotCount: 3
    })
  })
